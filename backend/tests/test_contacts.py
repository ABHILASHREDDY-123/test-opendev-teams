...
async def test_list_contacts():
    async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as ac:
        ...
        login_response = await ac.post("/auth/login", json={"mobile": "1234567890", "password": "test123"})
        token = login_response.json()['access_token']
        await ac.post("/contacts", json={"name": "Contact1", "mobile": "1111111111"},
                      headers={"Authorization": f"Bearer {token}"}))
        await ac.post("/contacts", json={"name": "Contact2", "mobile": "2222222222"},
                      headers={"Authorization": f"Bearer {token}"}))
...
async def test_update_contact():
    async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as ac:
        ...
        login_response = await ac.post("/auth/login", json={"mobile": "1234567890", "password": "test123"})
        token = login_response.json()['access_token']
        create_response = await ac.post("/contacts", json={"name": "Original Name", "mobile": "9876543210"},
                                        headers={"Authorization": f"Bearer {token}"}))
        contact_id = create_response.json()['id']
        response = await ac.put(f"/contacts/{contact_id}", json={"name": "Updated Name"},
                                headers={"Authorization": f"Bearer {token}"}))
        assert response.status_code == 200
...
async def test_delete_contact():
    async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as ac:
        ...
        login_response = await ac.post("/auth/login", json={"mobile": "1234567890", "password": "test123"})
        token = login_response.json()['access_token']
        create_response = await ac.post("/contacts", json={"name": "To Delete", "mobile": "9876543210"},
                                        headers={"Authorization": f"Bearer {token}"}))
        contact_id = create_response.json()['id']
        delete_response = await ac.delete(f"/contacts/{contact_id}", headers={"Authorization": f"Bearer {token}"}))
...
async def test_contacts_isolation():
    async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as ac:
        ...
        await ac.post("/contacts", json={"name": "User1 Contact", "mobile": "1111111111"},
        ...
        await ac.post("/contacts", json={"name": "User2 Contact", "mobile": "2222222222"},
        ...
        assert response1.json()[0]['name'] == 'User1 Contact'
        assert response2.json()[0]['name'] == 'User2 Contact'
...
async def test_update_other_users_contact():
    async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as ac:
        ...
        login1_response = await ac.post("/auth/login", json={"mobile": "1234567890", "password": "test123"})
        token1 = login1_response.json()['access_token']
        create_response = await ac.post("/contacts", json={"name": "User1 Contact", "mobile": "1111111111"},
                                        headers={"Authorization": f"Bearer {token1}"}))
        contact_id = create_response.json()['id']
        ...
        login2_response = await ac.post("/auth/login", json={"mobile": "9999999999", "password": "test123"})
        token2 = login2_response.json()['access_token']
        response = await ac.put(f"/contacts/{contact_id}", json={"name": "Hacked Name"},
                                headers={"Authorization": f"Bearer {token2}"}))
